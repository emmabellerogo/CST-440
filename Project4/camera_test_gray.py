...
def preprocess_crop(crop):
    """
    Resize crop to 96x96, convert to grayscale, normalise to [0,1].
    Matches training pipeline exactly — grayscale single channel.
    """
    resized = cv2.resize(crop, (IMG_SIZE, IMG_SIZE))
    gray    = cv2.cvtColor(resized, cv2.COLOR_RGB2GRAY)  # crop arrives as RGB
    normed  = gray.astype(np.float32) / 255.0
    return normed.reshape(1, IMG_SIZE, IMG_SIZE, 1)      # (1, 96, 96, 1)
